...
function oxy_admin_wcm_init(){
    
    add_filter( 'manage_edit-product_columns', 'oxy_admin_wcm_brands_edit_columns' );
    add_action( 'manage_product_posts_custom_column', 'oxy_admin_wcm_brands_custom_columns' , 3 );
    
}

add_action('admin_init','oxy_admin_wcm_init');
function oxy_admin_wcm_brands_edit_columns($existing_columns){
    $columns = array();
    $columns['brands'] = __('Brands','oxy');
    
    return array_merge($columns, $existing_columns);
}
function oxy_admin_wcm_brands_custom_columns($column){
    global $post;
    $termlist = array();
    switch($column){
        case 'brands':
...
function wcm_sds_brands_thumbnail_id($term_id)
{
	...
	return (int)$wpdb->get_var( 
        "SELECT wsb.meta_value FROM {$wpdb->prefix}wcm_sds_brands wsb "
        ."LEFT JOIN {$wpdb->term_taxonomy} tt "
        ."ON wsb.term_taxonomy_id=tt.term_taxonomy_id "
...
function set_wcm_sds_brand_logo($params = array())
{
	...
	$count = $wpdb->get_var( 
        "SELECT COUNT(*) FROM {$wpdb->prefix}wcm_sds_brands wsb"
        ." LEFT JOIN {$wpdb->term_taxonomy} tt ON wsb.term_taxonomy_id=tt.term_taxonomy_id"
        ." WHERE tt.term_taxonomy_id={$term_taxonomy_id}"
        ." AND tt.taxonomy='brands' AND wsb.meta_key='{$meta_key}'"
        );
	
	if($count>0):
		
                $wpdb->query("update {$wpdb->prefix}wcm_sds_brands set meta_value='{$attach_id}' WHERE term_taxonomy_id={$term_taxonomy_id} and meta_key='{$meta_key}'");
	
	else:
	
                $wpdb->query("insert into {$wpdb->prefix}wcm_sds_brands(term_taxonomy_id, meta_key, meta_value) values($term_taxonomy_id, '{$meta_key}', '{$attach_id}')");	
...
function wcm_sds_brands_fields_save($term_id, $tt_id, $taxonomy)
{
	if(isset($_POST['wcm_sds_brand_logo'])):
		...
		set_wcm_sds_brand_logo(array('term_id' => $term_id,'term_taxonomy_id' => $tt_id, 'attach_id' => $_POST['wcm_sds_brand_logo']));
		
	endif;
}

function delete_oxy_term($tt_id){
    
    global $wpdb;
    $q = $wpdb->query("DELETE FROM {$wpdb->prefix}wcm_sds_brands WHERE 
        term_taxonomy_id=$tt_id");
    
    return $q;
    
}
add_action ('delete_term_taxonomy','delete_oxy_term');
